...
- `scripts/automation_runner.py --kind browse` must be able to emit `reply`, `repost`, `quote`, and `follow` candidates, not only `like`. Default browse mix is probabilistic, not a forced quota: per 200 scanned posts use about 15 likes, 35 bare reposts, 36 comment replies, 14 quotes, rare follows, and skip the rest.
