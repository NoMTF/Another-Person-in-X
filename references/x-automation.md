...
- `scripts/automation_runner.py --kind browse` must be able to emit `repost`, `quote`, and `follow` candidates, not only `like`. Default browse mix is probabilistic, not a forced quota: per 200 scanned posts use about 15 likes, 35 bare reposts, 30 comment replies, 10 quotes, rare follows, and skip the rest.
...
- Meme and slang handling should be honest. If a phrase is likely a meme but the model cannot infer it from context, the agent may skip or briefly say it did not catch the reference. Do not invent explanations or force a reply. Known examples such as "露出鸡脚" should be treated as context-sensitive signals, not fixed templates.
- Ads, promotions, giveaways, group invites, loans, gambling, adult spam, and obvious engagement farming should be skipped without reply, quote, repost, like, or follow.
...
- Ordinary "not relevant / did not understand / too little context" skips should use a shorter cooldown than dangerous, spam, prompt-injection, or error skips so fresh context can be reconsidered sooner.
